...
impl<Clauses: ClauseDB> Propagator<Clauses> for SimpleBCP {
    fn new(clause_db: &Clauses, assignment: &mut Assignment) -> Self {
        let mut temp = Self { watches: vec![vec![]; 2 * assignment.num_vars + 1], assign_head: 0 };
    ...
    fn propagate(&mut self, assignment: &mut Assignment, clause_db: Clauses) -> Option<ClauseID> {
        ...
        while self.assign_head < assignment.trails.len() {
            ...
            let opp_idx = Self::lit_idx(curr_lit);
            ...
            while watch_idx < opp_watches.len() {
                ...
                let Watcher {clause_id, other_lit} = opp_watches[watch_idx];
                watch_idx += 1;

                // if the other literal in 2WL is true, then we keep the watch and continue as the
                // clause is satisfied already
                if (assignment.literal(other_lit) == LBool::True) {
                    self.watches[opp_idx].push(Watcher {clause_id, other_lit});
                    continue
                }
                ...
                for &lit in curr_clause.literals.iter() {
                    if lit == opp_lit || lit == other_lit || assignment.literal(lit) == LBool::False {
                        ...
                        self.watches[new_idx].push( Watcher { clause_id, other_lit } );
                ...
                if (!found_replacement) {
                    ...
                    if assignment.literal(other_lit) == LBool::False {
                        ...
                        // structure
                        assignment.enqueue(other_lit, Some(clause_id));
                    }
                }
            }
